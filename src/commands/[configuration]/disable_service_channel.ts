...
export default {
  type: 'slash',
  description: 'Manage disabled service channels',
  ...
  options: [
    ...
    },
    {
      ...
      description: 'Select a channel to disable',
      required: true,
    },
  ],
  ...
  callback: async ({ interaction }) => {
    ...
    await database.disableServiceChannel(interaction?.guildId?.toString() as string, service, channel);

    return { content: `Disabled service **${service}** in channel <#${channel}>`, flags: MessageFlags.Ephemeral };
  },
} as DzajCommand;
